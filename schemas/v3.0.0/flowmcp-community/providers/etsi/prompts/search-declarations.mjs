export const content = `Search Standard Essential Patent declarations in the ETSI IPR database using Playwright browser automation.

## Prerequisites
- Playwright MCP server must be available
- User must be present (AJAX-heavy interface, potential session issues)
...
**By company:** Select [[companyName]] from the "Declaring Companies" multi-select list (432 companies).
Hold CTRL to select multiple companies.

**By standard/project:** If [[standard]] is provided, first select the ETSI Project from the "ETSI Projects" list (272 projects, loaded via AJAX). Then select the specific work item from the dependent "Work Item / Standard / Specification No." list (updates dynamically).

**By date range:** If [[dateFrom]] and [[dateTo]] are provided, use the date pickers (format: dd/MM/yyyy).

**By declaration type:** Select from dropdown if filtering for "General Declaration" or "Information Statement and Licensing Declaration".

**By licensing position:** Select "Prepared" or "Not prepared" if relevant.
